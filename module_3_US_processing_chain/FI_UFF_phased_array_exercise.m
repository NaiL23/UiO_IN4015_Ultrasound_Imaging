...
for t = 1:N_transmits
    ...
    tx_delays = repmat(radial_distance/c, [N_elements 1])';
    rx_delays = hypot(radial_distance * sin(angles(t)) - x_element_position, radial_distance * cos(angles(t)) - z_element_position)'/c;
    delays(t, :, :) =  tx_delays + rx_delays - offset(t);
    ...
    for r = 1:N_elements
        ...
        img(:, t) = img(:, t) + interp1(sample_time, rfData(:, r, t)', delays(t, :, r), 'linear', 0)';
    end
end
...
mid.receive_apodization.f_number = 2.0;  %<--- UNCOMMENT AND FILL OUT THIS LINE
