...
coherent_compounding_signal = sum(image_matrix, 3);
coherent_compounding = db(abs(coherent_compounding_signal/max(max(coherent_compounding_signal))));           % <---- Implement coherent compounding here

% Create incoherent compounded image
incoherent_compounding_signal = sum(abs(image_matrix), 3);
incoherent_compounding = db(abs(incoherent_compounding_signal/max(max(incoherent_compounding_signal))));
...
if all(all(abs(coherent_compounding_USTB - coherent_compounding) < 10^-4))
    disp("Correct Coherent Compounding Implementation");
end

%% Compare your implementation of incoherent compounding to the USTB
if all(all(abs(incoherent_compounding_USTB - incoherent_compounding) < 10^-4))
    disp("Correct Incoherent Compounding Implementation");
end
...
% angles_first_sum = 1:2:channel_data.N_waves;
% angles_second_sum = 2:2:channel_data.N_waves;
...
mix_compounding_signal = abs(sum(image_matrix(:,:,angles_first_sum), 3)) + abs(sum(image_matrix(:,:,angles_second_sum), 3));
mix_compounding = db(abs(mix_compounding_signal/max(max(mix_compounding_signal)))); % <---- Implement coherent compounding here. You might need more than one line
...
%     incoherent_compounding_signal = b_data_incoherent.data;
    ...
    std_background_single = std(abs(single_image_signal(idx_background(:))).^2, 1);
    std_ROI_single = std(abs(single_image_signal(idx_ROI(:))).^2, 1);
    
    std_background_coherent = std(abs(coherent_compounding_signal(idx_background(:))).^2, 1);
    std_ROI_coherent = std(abs(coherent_compounding_signal(idx_ROI(:))).^2, 1);
    
    std_background_incoherent = std(abs(incoherent_compounding_signal(idx_background(:))).^2, 1);
    std_ROI_incoherent = std(abs(incoherent_compounding_signal(idx_ROI(:))).^2, 1);

    std_background_mix = std(abs(mix_compounding_signal(idx_background(:))).^2, 1);
    std_ROI_mix = std(abs(mix_compounding_signal(idx_ROI(:))).^2, 1);
    
    CNR_single = abs(mean_ROI_single - mean_background_single) / sqrt(std_ROI_single^2 + std_background_single^2)
    CNR_coherent = abs(mean_ROI_coherent - mean_background_coherent) / sqrt(std_ROI_coherent^2 + std_background_coherent^2)
    CNR_incoherent = abs(mean_ROI_incoherent - mean_background_incoherent) / sqrt(std_ROI_incoherent^2 + std_background_incoherent^2)
    CNR_mix = abs(mean_ROI_mix - mean_background_mix) / sqrt(std_ROI_mix^2 + std_background_mix^2)
